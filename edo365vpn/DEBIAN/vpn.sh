...
echo "Ghost CMS install and config helper"
echo -n "Enter CA:"
read CA
echo -n "Enter hostname (vpn...):"
read HOSTNAME
...
EOF
	...
	cat <<EOF |  sudo vi -c "vsplit /etc/ipsec.secrets" -O -
: RSA "server-key.pem"
...
setup_ufw (){
	...
	cat <<EOF |  sudo vi -c "vsplit /etc/ufw/before.rules" -O -
*nat
...
EOF
	...
	cat <<EOF |  sudo vi -c "vsplit /etc/ufw/sysctl.conf" -O -
